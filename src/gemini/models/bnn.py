...
class BayesianNeuralNetwork(NetworkModel):
	...
	def build_graph(self):
		...
		with self.graph.as_default():
			...
			self.layers.append(layer(self.targets_shape, activation=act_out))
			self.neural_net = tf.keras.Sequential(self.layers)
			self.loc = self.neural_net( self.tf_x )
			self.scale = tf.nn.softplus(tf.Variable(tf.ones(self.loc.get_shape())))
	...
	def train(self, train_features, train_targets,
		...
		# compute the stats
		self.dataset_stats(train_features, train_targets, None, None)
	...
	def run_prediction(self, features, scaled=False, num_samples=10):
		if not scaled:
		...
		pred_mu     = np.empty((num_samples, features.shape[0], self.targets_shape))
		pred_std    = np.empty((num_samples, features.shape[0], self.targets_shape))
		...
		for batch_iter, size in enumerate(res):
			...
			for _ in range(num_samples):
				predic_mu, predic_std = self.sess.run([self.loc, self.scale], feed_dict = {self.tf_x: X_batch})
				pred_mu[_, start:stop] = predic_mu[:size]
				pred_std[_, start:stop] = predic_std[:size]
		mean_pred, std_pred = np.mean(pred_mu, axis = 0), np.std(pred_mu, axis = 0)
		...
		# scale the targets
		mean_pred_raw = self.get_raw(mean_pred, 'targets', self.target_scaling, 'slow')
